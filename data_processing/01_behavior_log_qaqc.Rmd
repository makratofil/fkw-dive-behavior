...
p95 <- filter(behavior_qc, DeployID == "PcTag095")
p96 <- filter(behavior_qc, DeployID == "PcTag096")
p97 <- filter(behavior_qc, DeployID == "PcTag097")
p99 <- filter(behavior_qc, DeployID == "PcTag099")
...
for(i in 1:length(status_list)){
  ...
  crc_table <- data.frame(
    ...
    maxZeroDepthOffset = max(abs(crc$ZeroDepthOffset), na.rm = T)
  )
...
All tag files meet acceptable levels of depth and zero-depth-max values in their status files, so no need to remove any records based on status file readings. Note that the absolute value of the ZeroDepthOffset is reported. 
...
for(i in 9:12){
  ...
  for (c in levels(ser$hr_id)) {
    ...
    max_depth <- round(max(behav$DepthMax, na.rm = T), digits = -2) + 100
...
saveRDS(behavior_final, here("pipeline","qaqcd_behavior_data", "PcTags_SPLASH_thru099_behavior_logs_qaqcd_2025Oct02.rds"))
write.csv(behavior_final, here("pipeline","qaqcd_behavior_data", "PcTags_SPLASH_thru099_behavior_logs_qaqcd_2025Oct02.csv"),
          row.names = F)

behavior_final <- readRDS(here("pipeline","qaqcd_behavior_data", "PcTags_SPLASH_thru099_behavior_logs_qaqcd_2025Oct02.rds"))
